...
string NodeAttrHelper::get(const std::string &key, string def_val) const {
    for (int i = 0; i < node_.attribute_size(); i++) {
        const ONNX_NAMESPACE::AttributeProto &attr = node_.attribute(i);
        if (attr.name() == key) {
            return attr.s();
...
vector<int> NodeAttrHelper::get(const std::string &key,
                                vector<int> def_val) const {
    if (!has_attr(key)) {
        return def_val;
    }
    std::vector<int> v;
