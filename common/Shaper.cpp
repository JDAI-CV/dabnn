...
void Shaper::Binarize(css &input_name, css &output_name) {
    shape_map_[output_name] = shape_map_.at(input_name);
}

void Shaper::Split(const std::string &input_name,
                   const std::vector<std::string> &output_names,
                   uint32_t axis) {
    auto dimen = shape_map_[input_name];
    if (dimen[axis] % output_names.size() != 0) {
        throw string("Can't split evenly");
    }
    for (size_t i = 0; i < dimen.size(); i++) {
        if (i == axis) {
            dimen[i] /= output_names.size();
        }
    }
    for (const auto &output : output_names) {
        shape_map_[output] = dimen;
    }
}

void Shaper::Shuffle(css &input_name, css &output_name) {
    shape_map_[output_name] = shape_map_.at(input_name);
}
